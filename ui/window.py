...
class MaktabatiApp:
    ...
    def _build_header(self) -> None:
        ...
        tk.Label(header, text="Gestion simple et elegante de votre bibliotheque",
                 font=("Calibri", 12), fg="#f5dfc3", bg="#6b4226").pack(anchor="w", pady=(6, 0))
    ...
    def _build_content(self) -> None:
        ...
        right_panel = tk.Frame(container, bg="#fffaf3", bd=1, relief="solid", padx=18, pady=18)
        right_panel.pack(side="right", fill="both", expand=True, padx=(18, 0))

        tk.Label(left_panel, text="Actions principales", font=("Georgia", 18, "bold"),
                 fg="#4b2e1f", bg="#f4efe6").pack(anchor="w", pady=(0, 14))

        tk.Button(left_panel, text="Ajouter un livre", font=("Calibri", 13, "bold"),
                  bg="#6b4226", fg="white", activebackground="#5e4432",
                  activeforeground="white", bd=0, padx=18, pady=12, cursor="hand2",
                  command=self._open_add_form).pack(fill="x", pady=7)

        tk.Button(left_panel, text="Quitter", font=("Calibri", 13, "bold"),
                  bg="#7a5c46", fg="white", activebackground="#5e4432",
                  activeforeground="white", bd=0, padx=18, pady=12, cursor="hand2",
                  command=self.root.destroy).pack(fill="x", pady=7)

        count_card = tk.Frame(left_panel, bg="#fffaf3", bd=1, relief="solid", padx=14, pady=14)
        count_card.pack(fill="x", pady=(22, 0))
        tk.Label(count_card, text="Livres disponibles", font=("Georgia", 14, "bold"),
                 fg="#4b2e1f", bg="#fffaf3").pack(anchor="w")
        tk.Label(count_card, textvariable=self.count_var, font=("Calibri", 12, "bold"),
                 fg="#6b4226", bg="#fffaf3", pady=8).pack(anchor="w")

        tk.Label(right_panel, text="Liste des livres", font=("Georgia", 16, "bold"),
                 fg="#4b2e1f", bg="#fffaf3").pack(anchor="w")
        tk.Label(right_panel, text="Les livres enregistres s'affichent automatiquement ici.",
                 font=("Calibri", 12), fg="#6c5a4c", bg="#fffaf3").pack(anchor="w", pady=(6, 12))

        list_frame = tk.Frame(right_panel, bg="#f8efe3", padx=10, pady=10)
        list_frame.pack(fill="both", expand=True)

        self.books_listbox = tk.Listbox(list_frame, font=("Calibri", 12),
                                        bg="#fffdf9", fg="#3e3128", bd=0, highlightthickness=0)
    ...
    def _load_books(self) -> list:
        ...
        try:
            ...
            return []
        # FIX: storage saves {"book": [...]}, not a plain list
